...
var gauges = (
        function () {
            ...
            var strings = LANG.PL,         // Set to your default language. Store all the strings in one object
                ...
                gaugeGlobals = {
                    minMaxArea: 'rgba(112,196,187,0.7)', // area sector for today's max/min. (red, green, blue, transparency)
                    windAvgArea: 'rgba(132,212,134,0.0)',
                    windVariationSector: 'rgba(112,196,187,0.3)', // only used when rose data is shown on direction gauge
                    frameDesign: steelseries.FrameDesign.GLOSSY_METAL,
                    background: steelseries.BackgroundColor.BRUSHED_STAINLESS,
                    foreground: steelseries.ForegroundType.TYPE2,
                    pointer: steelseries.PointerType.TYPE12,
                    ...
                    lcdColour: steelseries.LcdColor.SECTIONS,
                    ...
                    function init() {
                        ...
                        function update() {
                            ...
                            if (data.pressunit === 'hPa' || data.pressunit === 'mb') {
                                //  default min range 960-1050 - steps of 5 hPa
                                cache.minValue = Math.min(nextLowest(cache.recLow - 5, 5), gaugeGlobals.baroScaleDefMinhPa);
                                cache.maxValue = Math.max(nextHighest(cache.recHigh + 5, 5), gaugeGlobals.baroScaleDefMaxhPa);
                                dps = 1; // 1 decimal place
                            } else if (data.pressunit === 'kPa') {
                                //  default min range 99-105 - steps of 1 kPa
                                cache.minValue = Math.min(nextLowest(cache.recLow - 2, 1), gaugeGlobals.baroScaleDefMinkPa);
                                cache.maxValue = Math.max(nextHighest(cache.recHigh + 2, 1), gaugeGlobals.baroScaleDefMaxkPa);
                                dps = 2;
                            } else {
                            ...
                            if (config.showWindVariation) {
                                ...
                                require.config({
                                    paths: {
                                        ...
                                        windGauge: "./scripts/wind_gauge_new/wind_gauge"
                                    }
                                });

                                require(["windGauge"], function (windGauge) {

                                    const linesPlaceholder = document.getElementById("wind-container");
                                    windGauge.showWind(linesPlaceholder, data.avgbearing, cache.avgKnots);
                ...
                createCloudBaseSections = function (metric) {
                    ...
                    if (metric) {
                        section = [
                            ...
                            steelseries.Section(5500, 12500, 'rgba(19, 50, 186, 0.5)')
                        ];
                    } else {
                        section = [
